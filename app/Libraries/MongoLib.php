    ...
    function __construct($selectedDB = 'default')
    {
        try {
            ...
            foreach ($this->mongoConnectionInfos->dbInfo as $key => $dbInfo) {
                if ($key === $selectedDB) {
                    $this->m = new client(
                        $this->mongoConnectionInfos->dbInfo[$key]->srv . "://{$this->mongoConnectionInfos->dbInfo[$key]->hostname}:{$this->mongoConnectionInfos->dbInfo[$key]->port}/{$this->mongoConnectionInfos->dbInfo[$key]->db}",
                        [
